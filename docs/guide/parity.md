...
| File copy-in | A file copied to a guest path before start is present, with identical content, the instant the workload's own process can see it. |
...
| Capabilities (`hardwareIsolated` / `checkpoint`) | `msb`: `hardwareIsolated: true`, `checkpoint: false`. `docker`: `hardwareIsolated: false`, `checkpoint: true`. Identical values, identical field names, in every implementation. |
...
| Checkpoint gating | `checkpoint()` throws a typed unsupported error before any backend call when `capabilities.checkpoint` is `false`, in every implementation; where it's supported (docker today), the restored container is ordinary in every respect — fresh ports, normal reaping, normal `stop()`. |
